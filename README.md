...
- `scripts/workmon-agent-0.4.0/` — worker-side WorkMon agent package consumed by `scripts/local-provision.sh`; captures X11 screenshots, encrypts with the shared age public recipient, and writes `.age` files to the local spool.
- `scripts/admin/copy_key.sh` — admin-side helper that copies the WorkMon collector `public.key` to a worker over SSH.
- `workmon/collect/` — admin-laptop collector package that pulls encrypted `.age` screenshots from workers, decrypts locally, and uploads PNG files to an SMB share.
