...
A migration of `scripts/local-provision.sh` v2.2 to Ansible inside the larger department VPS/thin-client repository. Push model: the playbook runs from the admin laptop and configures thin clients over SSH. A single run can provision the whole fleet in parallel.
...
| `step_workmon` | role `workmon` (stub for now) |
...
| passwords in the config | `.../thin_clients/vault.yml.example` → local encrypted `vault.yml` |
...
# Create a real local Vault file from the committed template (the real vault.yml is ignored by git):
...
inventory/
  ...
  group_vars/thin_clients/
    ...
    vault.yml.example          committed template; copy to ignored encrypted vault.yml
...
All behavior is preserved: the tunnel is not brought up during the run, passwords aren't overwritten on reruns, WG keys are generated on the machine itself and reused, x11vnc listens only on wg0, units are enabled on boot without starting in the current session.
